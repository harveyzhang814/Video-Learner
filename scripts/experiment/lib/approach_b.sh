#!/bin/bash
# approach_b.sh — Hybrid: single session, summary re-passes full article (same prompt as C but same session)
# Usage: bash approach_b.sh --transcript <path> --focus <str> --output-lang <str> --out-dir <dir>
...
echo "[B] Building article prompt..."
...
echo "[B] Creating single session..."
SID="$(opencode_create_session "exp-b-$$")"

echo "[B] Msg 1: generating article..."
opencode_send_msg "$SID" "$ARTICLE_PROMPT_FILE" \
  "$OUT_DIR/article.md" "$OUT_DIR/_metrics_article.json"
rm -f "$ARTICLE_PROMPT_FILE"
echo "[B] article done ($(wc -c < "$OUT_DIR/article.md") bytes)"

echo "[B] Building summary prompt (full article re-pass)..."
...
echo "[B] Msg 2: generating summary (same session, article re-passed)..."
opencode_send_msg "$SID" "$SUMMARY_PROMPT_FILE" \
  "$OUT_DIR/summary.md" "$OUT_DIR/_metrics_summary.json"
rm -f "$SUMMARY_PROMPT_FILE"
echo "[B] summary done ($(wc -c < "$OUT_DIR/summary.md") bytes)"

echo "[B] Writing metrics.json..."
