...
│       ├── orchestrator.js    # 适配器：委托 core/orchestrator（createTask / runTask / runStep 等）
│       └── renderer/
...
### 4.1.1 `force` 参数与各 Step（编排层 → 脚本）

**`force` 从哪来**

1. **创建任务**（`POST /api/tasks` 或 GUI 等价流程）：写入 SQLite / 内存中的 **`task.params.force`**（布尔或 0/1 语义）。之后 **`runTask`** 调度到 **`video` / `audio`** 时，会把该值传入对应 **`runStep(..., { force: task.params.force })`**（见 `core/orchestrator/index.js`）。
2. **单次执行某步**（`POST /api/tasks/:taskId/steps/:stepName/run` 的 body **`force`**）：仅影响本次 **`runStep`** 的 `options.force`，**不会**自动回写 `task.params.force`。
3. **`reset_scope: downstream`**：HTTP 层当前对下游调度调用 **`runTask(taskId, { rootDir })`**，**不**把本次请求 body 里的 `force` 合并进任务参数；是否「强制重下媒体」仍主要取决于**创建任务时**的 `task.params.force`，除非后续实现显式透传。

**编排层实际传参**（`runStep` 内 `switch (stepName)`）

- 仅 **`video`**、**`audio`** 会把 `force` 编成脚本的最后一个参数 **`'1'` / `'0'`**，调用形如 **`[url, dir, id, forceBit]`**。
- **`fetch` / `subs` / `vtt2md` / `md2vtt` / `article` / `summary`**：**不**接收、不传递 `force`；这些步是否跳过或覆盖产物，由**各自脚本**（例如字幕「已有同名 `.vtt` 则跳过下载」）或 **`reset_scope`** 重置步骤状态决定，**与 `force` 无关**。

**各 Step 行为摘要**

| Step | 是否传入 `force` | 行为说明 |
|------|------------------|----------|
| **video** | 是（第 4 参） | `scripts/download_video.sh`：`FORCE=0` 且已存在足够大的 `media/video.mp4` 时**跳过下载**（并走 skipped 类语义）；`FORCE=1` 时**不跳过**，重新拉取。 |
| **audio** | 是（编排传第 4 参） | `scripts/download_audio.sh` **注释**写的是「第 3 参为 `FORCE`」，但编排传入顺序与 **`download_video.sh` 不一致**：脚本内为 `FORCE="${3:-0}"`，而 **`$3` 实际为任务 `id`**，`$4` 才是 `force` 位且**脚本未读取**。因此**任务级 / `runStep` 的 `force` 目前无法按设计控制「已有 `audio.m4a` 是否跳过」**；维护时应将 `download_audio.sh` 与 `download_video.sh` 对齐为 **`FORCE="${4:-0}"`**（并保留 `url, dir, id`），或改为与 video 相同的四参约定。在此之前，文档以**实际代码**为准：音频步的 `force` **语义未正确落地**。 |
| 其余 | 否 | 不受 `force` 影响。 |

**GUI 重试弹窗**：确认重试时会对请求附带 **`force: true`**，意图是在重跑 **video/audio** 时尽量**强制重新下载**（在 **video** 上生效；**audio** 受上表已知问题影响，修复脚本前可能仍与预期不一致）。
...
- 对失败的 Step 重试：弹窗内可选「自动执行后续步骤」；未勾选时 `POST .../steps/:stepName/run` 带 **`reset_scope: step`**（先重置该步再跑），勾选时为 **`reset_scope: downstream`**（见第十二节 `reset_scope` 子节）；
...
| POST | `/api/tasks/:taskId/steps/:stepName/run` | 执行指定步骤。body 可选：`focus`、`force`；**`reset_scope`**（已实现）：`off`（默认）\| `step` \| `downstream`。语义见 [`docs/plans/2026-03-22-resume-from-step-design.md`](plans/2026-03-22-resume-from-step-design.md)。 |
...
与「只执行一步、不改其它步状态」及「重置后再跑」共用 **同一路径**，由 body 字段 **`reset_scope`** 区分（设计见 [`plans/2026-03-22-resume-from-step-design.md`](plans/2026-03-22-resume-from-step-design.md)）：
...
**鉴权**：与 `POST /api/tasks` 相同，本阶段路由层**不**额外要求 token（与历史行为一致）。

**`force`（body 可选）**：仅通过 **`runStep`** 作用于 **`video` / `audio`** 脚本；与其它 Step 无关；与 **`reset_scope`**、`task.params.force`、**`downstream`** 下是否透传的关系见 **§4.1.1**。

#### `reset_scope` 自动化测试（全 Step × 双入口）

- **命令**：`npm run test:reset-scope`（已并入 `npm run test:orchestrator:unit` / `npm run test:agent:core` 链条中的 orchestrator 单元部分）。
- **覆盖**：
  - **`tests/reset-scope-all-steps-http.test.js`**：对 **`ALL_STEPS` × `mode` ∈ {`transcript`,`both`,`video`,`audio`}** 分别请求 **`reset_scope: downstream`** 与 **`reset_scope: step`**（后者对 `fetch`/`video`/`audio`/`subs` 使用短时 **`runStep` stub**，避免拉起 yt-dlp；对其余步断言 A 层快速失败 **400**）。非法锚点（`excludedByMode`）断言 **400** + `BAD_ANCHOR_MODE`。未知 step 断言 **404**。
  - **`tests/service-client-reset-scope-all-steps.test.js`**：同一矩阵通过 **`electron/src/renderer/service-client.js`**（与 GUI 相同的 HTTP 客户端）调用，**400/404** 路径以 **throw** 形式断言（与 renderer 一致）。
- **`downstream`** 在测试中不跑真实整条线：`createApp({ runTaskForDownstream: async () => {} })` 注入空实现（仅测 HTTP 契约与 `reset_steps` 与 **`getDownstreamClosure`** 一致）。生产路径仍为 **`orchestrator.runTask`**。
- **任务 `mode` 持久化**：`tasks.mode` 列（迁移自 SQLite）在 **`createTask`** 时写入，**`loadTaskFromDb`** 回填 **`params.mode` / `meta.mode`**，以便 HTTP 进程重启后 **`reset_scope` 锚点合法性**（`BAD_ANCHOR_MODE`）与 **`transcript` / `video` / `audio`** 一致。
...
3. **复用机制**：新增逻辑时优先复用已有输出，注意与 `force` / `FORCE`（脚本内变量名）、SQLite 状态保持一致；**`force` 语义与各 Step 对照表见 §4.1.1**。
4. **FOCUS 重要性**：任何与 summary 相关的改动，都要考虑没有 FOCUS、补充 FOCUS 之后、重复运行等场景。
5. **契约文档**：修改流水线的状态字段或输出结构时，务必同步更新 `CLAUDE.md` 与本文件中对应章节。
6. **Agent Service**：修改 `core/orchestrator` 或 HTTP 路由时，注意保持与 Electron 适配器及 SQLite 持久化约定一致；新增 API 或字段时可在本节「Agent HTTP Service」及子节（如 `reset_scope`）中补充说明。
