...
**规划中（第二阶段 HTTP）**：`POST /api/tasks/:taskId/resume-from/:stepName` — 按 DAG 将锚点步及其下游闭包重置为 `pending` 后再触发与 `POST /api/tasks` 相同的 `runTask` 调度；与单步 `POST .../steps/:stepName/run` 不同。**设计**见 [`docs/plans/2026-03-22-resume-from-step-design.md`](plans/2026-03-22-resume-from-step-design.md)；**实现**见 [`docs/plans/2026-03-22-resume-from-step-implementation.md`](plans/2026-03-22-resume-from-step-implementation.md)。路由以代码落地为准；GUI 接线为更后阶段。
