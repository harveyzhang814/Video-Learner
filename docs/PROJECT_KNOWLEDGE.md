...
- **`WRITING_ENGINE`**：单次运行覆盖写作引擎（文章/总结生成用），优先级高于 `scripts/settings.conf` 的默认值。
  - 支持：`claude` / `opencode`
  - 若未设置，则由 `scripts/llm_engine.sh` 读取 `scripts/settings.conf` 的 `WRITING_ENGINE_DEFAULT` 决定；若仍未设置或非法，回退为 `opencode`。
...
- **行为**：经 HTTP 创建任务并等待整条流水线完成，校验 `meta.json`、逐字稿、`article.md`/`summary.md` 存在且正文非未替换 prompt；再重跑 `summary` 步并断言同路径产出已更新。默认**不**删除 `work/<id>/`，便于人工打开 Markdown 核验；若需清理可设 `E2E_CLEANUP=1`。
