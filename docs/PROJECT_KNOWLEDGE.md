...
| POST | `/api/tasks/:taskId/steps/:stepName/run` | 执行指定步骤。body 可选：`focus`、`force`；**`reset_scope`**：`off`（默认，等同仅 `runStep`）\| `step`（先重置该步为 `pending` 再跑该步）\| `downstream`（先按 DAG 重置锚点及下游闭包再 fire-and-forget `runTask`）。详见 [`docs/plans/2026-03-22-resume-from-step-design.md`](plans/2026-03-22-resume-from-step-design.md)。 |
