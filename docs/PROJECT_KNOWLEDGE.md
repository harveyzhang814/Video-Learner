...
**规划中（第二阶段 HTTP）**：`POST /api/tasks/:taskId/resume-from/:stepName` — 按 DAG 将锚点步及其后继重置为 `pending` 后再 `runTask`；与单步 `POST .../steps/:stepName/run` 不同。设计见 [`docs/plans/2026-03-22-resume-from-step-design.md`](plans/2026-03-22-resume-from-step-design.md)；实现见同目录 `*-implementation.md`。路由以代码落地为准。
