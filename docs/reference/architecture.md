...
- **`runTask`（池式并发调度）**：`createTask` 之后由 HTTP 或 Electron 触发；内部用 `core/orchestrator/schedule.js` 循环计算就绪集，按主链/旁支优先级**并发**调用 `runStep`，上限 `VL_MAX_PARALLEL_STEPS`（默认 3）个在飞步骤，直至无步可调度。串行回退：设置 `VL_MAX_PARALLEL_STEPS=1`。
...
**YouTube 人机验证**：在 `~/.config/vdl/settings.conf` 中设置 `YT_DLP_COOKIES_BROWSER=chrome`（或 `safari`/`firefox`/`edge`），或设置 `YT_DLP_COOKIES_FILE=/path/to/cookies.txt`。
...
**字幕文件命名规则**（`transcript/subs/` 目录下）：

| 文件后缀 | 来源 | 预合并 |
|----------|------|--------|
| `*.original.vtt` | 平台人工字幕（`--write-subs`） | ❌ 时间戳已合理 |
| `*.auto.vtt` | 平台 AI 自动字幕（`--write-auto-subs`） | ✅ `vtt2md` 阶段执行预合并 |
| `*.asr.vtt` | 本地 Whisper ASR 转录输出 | ✅ `vtt2md` 阶段执行预合并 |

`vtt2md` 通过文件名后缀检测 AI 字幕并触发 `merge_ai_subs.py` 预合并，将 2–3 秒细粒度行合并为 3–6 秒句子级时间块，使中文字幕阅读速度匹配中文读者习惯。详见 `explanation/subtitle-translate-pipeline.md`。
...
| `WRITING_ENGINE_DEFAULT` | 全局默认写作引擎（写入 `~/.config/vdl/settings.conf`）；缺省或非法时回退为 `opencode` |
...
| `TRANSLATE_PAGE_SIZE` | `translate_subs.sh` 每页行数（默认 200） |
| `TRANSLATE_PARALLEL` | `translate_subs.sh` 最大并发 LLM 调用数（默认 5） |
| `VL_MAX_PARALLEL_STEPS` | 单任务最大并发步骤数（默认 3）；主链步骤优先占槽，旁支用余量 |
| `TRANSLATE_PAGE_TIMEOUT` | `translate_subs.sh` 单页超时秒数（默认 600） |
| `TRANSLATE_MIN_COVERAGE` | `translate_subs.sh` 最低覆盖率 %（默认 90） |
| `VTT2MD_MIN_BLOCK_SECS` | `vtt2md` AI 字幕预合并最短块时长秒（默认 3） |
| `VTT2MD_MAX_BLOCK_SECS` | `vtt2md` AI 字幕预合并最长块时长秒（默认 6） |
| `LLM_ENGINE_SCRIPT` | 覆盖 LLM 引擎脚本路径，供测试注入 mock（默认 `scripts/llm_engine.sh`） |
